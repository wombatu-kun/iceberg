...
| retry.strategy-impl                | (exponential backoff) | Fully-qualified name of a custom `org.apache.iceberg.util.BackoffStrategy` used to compute the wait between retries. Global to every `Tasks` retry site (commit, lock acquisition, status checks, REST scan polling, FileIO cleanup); there is no per-site override. Read from whichever properties map is local to the call site (table/view properties, catalog properties, FileIO properties, or Hadoop `Configuration`). When unset, the built-in exponential backoff with jitter is used. A built-in adaptive option is available at `org.apache.iceberg.util.AIMDBackoffStrategy` (additive-increase on failure, multiplicative-decrease on success) configured through the `retry.aimd.*` keys below |
| retry.aimd.min-ms                  | 100              | Floor for the cached delay used by `AIMDBackoffStrategy` |
| retry.aimd.max-ms                  | 60000 (1 min)    | Cap for the cached delay used by `AIMDBackoffStrategy` |
| retry.aimd.increase-step-ms        | 1000             | Per-failure additive step (in ms) used by `AIMDBackoffStrategy` |
| retry.aimd.decrease-factor         | 0.5              | Per-success multiplicative factor in `(0, 1]` used by `AIMDBackoffStrategy`; `1.0` disables the decrease |
